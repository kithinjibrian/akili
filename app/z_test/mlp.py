...
from app.a_mlp import NeuralNetwork
...
def demo_mnist() -> Tuple[NeuralNetwork, np.ndarray, np.ndarray, np.ndarray]:   
    ...
    nn.train(
        ...
        model_save_path='models/mnist/model',
    )
...
def demo_mnist_load():
    ...
    nn.load_best_model('models/mnist/model')
    ...
    print(f"Loaded Model Test Results:")
    print(f"Test Accuracy: {test_accuracy:.4f} ({test_accuracy*100:.2f}%)")
    print(f"Test Loss: {test_loss:.4f}")
